...
const ToReview = () => {
  ...
  const handleAddReview = async () => {
    try {
      setLoading(true);
      const res = await client.patch("/users/review", {
        reviewee: selectedReview?.reviewee?._id,
        rating,
        review,
      });
      ...
      setRating(0);
      setReview("");
      res.data && fetchToReview();
      res.data && setSelectedReview(null);
      res.data && alert("Review Added Successfully");
      setOpen(false);
  ...
  return (
    ...
    >
      ...
      <View style={{ width: "100%" }}>
        {toReview ? (
          <FlatList
            ...
            renderItem={({ item }) => (
              <ItemComponent
                item={{ ...item?.listing, userId: item?.reviewee }}
                type="column"
                onPress={() => {
                  setOpen(true);
      ...
      <Modal visible={open} animationType={"slide"} fullScreen>
        ...
        >
          <ScreenHeaderComponent
            title={`Review ${selectedReview?.reviewee?.name}`}
            hideModeToggle={true}
            backAction={() => {
              setOpen(false);
              setReview("");
          ...
          <Text>
            Note: You are reviewing for {selectedReview?.listing?.name}
          </Text>
          <View
            style={{
              flexDirection: "row",
          ...
          <View style={{ width: "100%", marginTop: 20 }}>
            {!error ? (
              ...
              >
                Note: You can't edit your review once submitted
              </Text>
            ) : (
              <Text
                style={{
