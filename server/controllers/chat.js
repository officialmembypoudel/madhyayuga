  ...
  try {
    ...
    if (chat) {
      ...
      chat.members[0] = {
        ...
        phone: user1.phone,
        expoPushToken: user1.expoPushToken,
      };
      chat.members[1] = {
        name: user2.name,
        email: user2.email,
        ...
        phone: user2.phone,
        expoPushToken: user2.expoPushToken,
      };
...
export const acceptDeal = async (req, res) => {
  try {
    ...
    const reviewer = updatedChat.members.filter(
      (member) => member.toString() !== updatedChat.listing.userId.toString()
    )[0];
